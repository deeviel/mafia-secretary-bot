...
export async function ensureVoiceConnectionReady(
  ...
  if (voiceReadyPromises.has(connectionKey)) {
    debugLog(
      `[Voice Connection] Awaiting existing ready check for key: ${connectionKey}...`,
    );
    const connection = await voiceReadyPromises.get(connectionKey)!;
    return connection;
  }
  ...
  const checkPromise = (async () => {
    ...
    if (!connection._hasListeners) {
        ...
        if (
          ...
          setTimeout(() => {
            if (connection.state.status === VoiceConnectionStatus.Signalling) {
              debugLog(
                `[Voice Connection] Primary connection stuck in signalling. Destroying aggressively.`,
              );
              try {
                connection.destroy();
              } catch (e) {}
      ...
      connection.on(VoiceConnectionStatus.Disconnected, async () => {
        try {
          ...
          debugLog(
            `[Voice Connection] Real disconnection detected for guild ${guildId}. Destroying connection to allow fresh reconnection on next use.`,
          );
          try {
            connection.destroy();
          } catch (e: any) {
            debugLog(
              `[Voice Connection] Destroy on disconnect failed: ${e.message}`,
            );
          }
        }
      });
    ...
    try {
      ...
      // Use 2s wait instead of 5s/15s so we can quickly self-heal if it stalls (Cloud Run UDP issue)
      ...
      debugLog(
        `[Voice Connection Stalled] Connection was unable to reach READY status within 2s. Current state: "${connection.state.status}". Reason: ${errStr}`,
      );
      ...
      newConnection.on("stateChange", (oldState: any, newState: any) => {
        ...
        if (
          ...
          setTimeout(() => {
            if (
              newConnection.state.status === VoiceConnectionStatus.Signalling
            ) {
              debugLog(
                `[Voice Connection] Connection stuck in signalling. Destroying aggressively.`,
              );
              try {
      ...
      try {
        ...
        debugLog(
          `[Self-Healing UNSUCCESSFUL] Ready state retry also timed out for channel "${channel.name}": ${retryErr.message}`,
        );
        try {
          newConnection.destroy();
        } catch (e) {}
  ...
  try {
    const connection = await checkPromise;
    return connection;
  } finally {
...
export function getOrCreateGuildPlayer(guildId: string, connection: any) {
  ...
  if (!player) {
    ...
    player.on("stateChange", (oldState: any, newState: any) => {
      debugLog(
        `Persistent Player state for ${guildId}: ${oldState.status} -> ${newState.status}`,
      );
      if (
...
async function playLocalFileInChannelForClient(
  ...
  // Failsafe: if the player is actually idle but our flag thinks it's playing, reset it so we don't get stuck
  if (
    player.state.status === AudioPlayerStatus.Idle &&
    guildIsPlaying.get(connectionKey)
  ) {
    debugLog(
      `[Failsafe] Player was idle but guildIsPlaying was true. Resetting flag.`,
    );
    guildIsPlaying.set(connectionKey, false);
  }
...
export async function playLocalFileInChannels(
  ...
  for (const channelId of channelIds) {
    ...
    if (
      ...
      try {
        const channel = await client2.channels
          .fetch(channelId)
          .catch(() => null);
        if (
          channel &&
...
export async function playAudioInVoiceChannels(
  ...
  for (const channelId of channelIds) {
    try {
      ...
      if (!connection) {
        debugLog(
          `Voice connection to channel ${channelId} failed to reach READY state (timeout or disconnected). Skipping playback for this channel.`,
        );
        continue;
      }
  ...
  if (playSuccessCount === 0 && channelIds.length > 0) {
    debugLog(
      "Voice connection failed: Could not establish a stable voice session to any requested channel.",
    );
  }
}
...
function startScheduleLoop(
  ...
  const interval = setInterval(() => {
    ...
    if (botNum === 1) {
      ...
      events
        .filter((e: ScheduledEvent) => {
          const isEnabled =
            e.autoTransferEnabled !== undefined
              ? e.autoTransferEnabled
              : globalSettings.autoTransferAtStart;
          return e.enabled && isEnabled;
        })
        .forEach((e: ScheduledEvent) => {
          const type =
            e.autoTransferType || globalSettings.autoTransferType || "delay";
          const absoluteTime =
            type === "absolute"
              ? e.autoTransferTime || globalSettings.autoTransferTime
              : null;
          if (type === "absolute" && absoluteTime) {
            if (curTimeStr === absoluteTime) {
              const runsToday =
                !e.days || e.days.length === 0 || e.days.includes(curDay);
              if (runsToday) {
    ...
    if (now - lastAutoJoinTime >= 10000) {
      ...
      const recentAudio1 = Array.from(lastAudioPlayTime.entries()).some(
        ([k, v]) => !k.endsWith("_bot2") && now - v < 5 * 60 * 1000,
      );
    ...
    const triggerAutoTransfer = () => {
      ...
      if (isEnabled) {
        const type =
          nextEvent.autoTransferType ||
          globalSettings.autoTransferType ||
          "delay";
        if (type === "delay") {
          const delayMins =
